...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const prompt = buildAnalysisPrompt(contentForAnalysis, channelContext, locale)
...
function buildAnalysisPrompt(content: string, channelContext: string, locale: string): string {
  const lang = locale === 'fr' ? 'français' : 'English'
  return `Analyste économique. ${channelContext}\n\n${content}\n\nAnalyse en ${lang}. JSON sans markdown :\n{"executiveTakeaway":"2-3 phrases","whyThisMatters":["point1","point2"],"immediateImplications":["impact1","impact2"],"secondOrderEffects":["effet1","effet2"],"regionalImplications":[{"region":"Nom","implications":["impl1"]}],"sectorExposure":[{"sector":"Nom","riskLevel":"high","notes":["note1"]}],"whatToWatch":["item1","item2"],"confidenceNote":"note"}`
}
